...
import { get_seller_profile } from "../../store/Reducers/sellerDetailsReducer";
import { ArrowLeft } from "lucide-react";

const AAPublicProfilePage = () => {
  ...
  useEffect(() => {
    dispatch(get_seller_profile(username));
  }, [username]);
    ...
    <div className="min-h-screen bg-gray-50 px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
          <div className="flex items-center space-x-4 sm:space-x-6">
            <div className="rounded-xl p-3 w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center shadow-sm">
              <img
                src={sellerInfo.agencyInfo.agencyLogo[0]}
                alt="Agency Logo"
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate max-w-xs sm:max-w-md">
                {truncateText(sellerInfo.agencyInfo.name, 25)}
              </h1>
              <p className="text-gray-600 mt-1 text-sm sm:text-base truncate max-w-xs sm:max-w-md">
                Username: {truncateText(sellerInfo.agencyInfo.username, 20)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm p-6 sm:p-8 mb-6 border border-gray-200">
        ...
        <p className="text-gray-700 leading-relaxed text-sm sm:text-base">
          {sellerInfo.agencyInfo.description}
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {sellerInfo.agencyInfo.mainAISpecializations.map(
                (item, index) => (
                  <div
                    key={index}
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {sellerInfo.agencyInfo.aiTools.map((item, index) => (
                <div
                  key={index}
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 border border-gray-200">
            ...
            <p className="text-gray-700 leading-relaxed">
              {sellerInfo.agencyInfo.previousProjects}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 border border-gray-200">
            ...
            <p className="text-gray-700 leading-relaxed">
              {sellerInfo.agencyInfo.additionalInfo}
            </p>
          </div>
        </div>
          ...
          {[
            {
              icon: UserGroupIcon,
              label: "Agency Size",
              value: `${sellerInfo.agencyInfo.teamSize} employees`,
            },
            {
              icon: CurrencyDollarIcon,
              label: "Pricing Model",
              value: sellerInfo.agencyInfo.pricingModel,
            },
            {
              icon: BuildingOfficeIcon,
              label: "Industry Expertise",
              value: sellerInfo.agencyInfo.industryExpertise,
            },
          ].map((item, index) => (
            <div
              key={index}
...
export default AAPublicProfilePage;
